...
import { exportAdminBackupCsv, importAdminBackupCsv, listAdminData } from "@/lib/admin-db";
...
function csvErrorResponse(error: unknown) {
  if (error instanceof z.ZodError) {
    return NextResponse.json({ error: error.issues[0]?.message || "Invalid CSV backup file." }, { status: 400 });
  }

  console.error("Admin CSV backup API failed:", error);
  const message = error instanceof Error ? error.message : "CSV backup request failed.";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = await importAdminBackupCsv(CsvRestoreSchema.parse(rawBody));
