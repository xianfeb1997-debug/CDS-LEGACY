...
import { generatePreviewLAS } from "@/lib/las-preview";
import { buildLASFileName, packLASDocx } from "@/lib/las";
...
export async function POST(request: Request) {
  try {
    ...
    const generated = await generatePreviewLAS(payload, payload.lesson, provider);
    const buffer = await packLASDocx(generated.las);
    const filename = buildLASFileName(payload, generated.las);
    ...
    const message = error instanceof Error ? error.message : "Unable to generate the preview LAS.";
    console.error("Preview LAS generation failed", { error: message });
